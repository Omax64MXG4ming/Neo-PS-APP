...
module.exports = {
	data: new SlashCommandBuilder()
		.setName("weeklylevel")
		.setDescription("Check this week's weekly level"),
	async execute(interaction) {
		await interaction.deferReply();

		try {
			const res = await fetch(M.host + "/bot/api/dailylevel.php?type=1");
			const search = await res.json();

			if (search.UP === "1970-01-01") return interaction.editReply("Sorry, this level is not found.");
			if (!search.id) return interaction.editReply("There is nothing set as weekly level.");
			...
			await interaction.editReply({ content: "This level for weekly", embeds: [embed] });
		} catch (err) {
			console.error(err);
			await interaction.editReply("An error occurred while fetching the weekly level.");
		}
	}
};
